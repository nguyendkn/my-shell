...
// Build the computer-use tool list. Real impl interpolates capability flags
// and the installed-app names into request_access description; the shim
// returns the static list and ignores the call args.
...
export function createComputerUseMcpServer(
  adapter?: {
    ...
    executor?: { capabilities?: unknown; listInstalledApps?: () => Promise<string[]> };
    isDisabled?: () => boolean;
  },
  _coordinateMode?: unknown,
) {
  ...
  return {
    // Connect to the given transport. The shim accepts the StdioServerTransport
    // (or equivalent) but discards it — the real impl wires up RPC dispatch
    // via the transport, but the shim only needs to log a warning.
  ...
  return async (
    ...
    switch (name) {
      ...
      case "list_granted_applications": {
        ...
        const names = apps.map((app) => app.displayName || app.bundleId || "unknown").join(", ");
