...
export class StructuredIO {
  ...
  private async processLine(line: string): Promise<StdinMessage | SDKMessage | undefined> {
    ...
    try {
      ...
      // The SDK schema uses an `unknown` placeholder for `message`; narrow
      // structurally to the role-bearing shape before reading `role`.
      const inner = message.message as { role?: string };
      if (inner.role !== "user") {
        exitWithMessage(`Error: Expected message role 'user', got '${inner.role}'`);
      }
      return message;
    } catch (error) {
  ...
  for await (const hookResult of hookGenerator) {
    if (
      ...
      if (decision.behavior === "allow") {
        ...
        // The SDK's `PermissionUpdate` and the runtime's `PermissionUpdate`
        // (PermissionUpdateSchema) describe the same wire shape but resolve
        // to two distinct nominal types — cast through Parameters<...> so
        // the consumer signatures stay authoritative.
        const permissionUpdates = (decision.updatedPermissions ?? []) as Parameters<
          typeof persistPermissionUpdates
        >[0];
