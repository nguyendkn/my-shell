...
// Discriminated by `ok`. The retry hint is on the failure variant — kept
// optional so the callers can `result.ok ? ... : new RetryableError(..., result.retryAfterMs)`
// without a separate undefined-check.
type RequestSuccess = { ok: true; retryAfterMs?: undefined };
type RequestFailure = { ok: false; retryAfterMs?: number };
...
export class CCRClient {
  ...
  async writeEvent(message: StdoutMessage): Promise<void> {
    if (message.type === "stream_event") {
      // StdoutMessage's stream_event variant types `event` as unknown (the
      // wire schema uses z.unknown() because the SDK type is unavailable at
      // schema-build time). At runtime it's always a BetaRawMessageStreamEvent;
      // cast at this boundary so coalesceTextDeltas can narrow on event.type.
    ...
    if (message.type === "assistant") {
      // The wire schema declares message via APIAssistantMessagePlaceholder
      // (unknown). At runtime it's the SDK assistant shape with at least
      // {id}, which is all clearStreamAccumulatorForMessage needs.
