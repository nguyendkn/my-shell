  ...
  if (Object.keys(configs).length === 0) {
    ...
    for (const { name, server, status } of results) {
      ...
      if (server.type === "sse") {
        ...
        // The discriminated union over server.type has stdio-shape with
        // command/args; non-strict mode doesn't narrow optional `type`,
        // so cast through the structural surface this branch reads.
        const stdio = server as { command?: string; args?: readonly string[] };
        const args = Array.isArray(stdio.args) ? stdio.args : [];
        // biome-ignore lint/suspicious/noConsole:: intentional console output
        console.log(`${name}: ${stdio.command ?? ""} ${args.join(" ")} - ${status}`);
      }
    }
  }
