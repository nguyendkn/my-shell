...
export function saveOAuthTokensIfNeeded(tokens: OAuthTokens): {
  ...
  try {
    ...
    // SecureStorage is a generic Record<string, unknown>; the runtime stores
    // OAuthTokens under claudeAiOauth. Narrow at the read boundary so the
    // subscription/rateLimitTier fallbacks type-check.
...
export const getClaudeAIOAuthTokens = memoize((): OAuthTokens | null => {
  ...
  try {
    const secureStorage = getSecureStorage();
    const storageData = secureStorage.read();
    // Cast at the secure-storage read boundary; the runtime stores OAuthTokens
    // under claudeAiOauth. accessToken is the required-field invariant
    // checked below.
 ...
 */
export type OrgValidationResult = { valid: boolean; message?: string };
