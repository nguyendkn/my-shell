...
const AWS_STS_CLIENT = "@aws-sdk/client-sts";
const AWS_CREDENTIAL_PROVIDERS = "@aws-sdk/credential-providers";
...
export async function checkStsCallerIdentity(): Promise<void> {
  const { STSClient, GetCallerIdentityCommand } = await import(AWS_STS_CLIENT);
...
export async function clearAwsIniCache(): Promise<void> {
  try {
    ...
    const { fromIni } = await import(AWS_CREDENTIAL_PROVIDERS);
