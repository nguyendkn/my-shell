...
/**
 * FPT Smart Cloud branding: khi user chưa chọn theme, default về `fpt`
 * (dark) hoặc `fpt-light` thay vì `dark`/`light` vanilla — đảm bảo CLI
 * ra ngay trong brand colors không cần `/theme` thủ công.
 */
function toFptVariant(theme: SystemTheme): ThemeName {
  return theme === "light" ? "fpt-light" : "fpt";
}
...
export function resolveThemeSetting(setting: ThemeSetting): ThemeName {
  if (setting === "auto") {
    return toFptVariant(getSystemThemeName());
  }
  return setting;
}
