...
class Project {
  ...
  async insertMessageChain(
    ...
    return this.trackWrite(async () => {
      ...
      for (const message of messages) {
        ...
        // The spread of `message` carries Message-union fields; combined with
        // parentUuid/isSidechain/etc this satisfies TranscriptMessage shape at
        // runtime, but TS's union-spread inference can't prove every variant
        // covers `uuid` (some MessageBase variants make it optional). Cast at
        // the assignment boundary — the runtime invariant is enforced by the
        // calling sites that only feed in messages with a uuid.
  ...
  async appendEntry(entry: Entry, sessionId: UUID = getSessionId() as UUID) {
    ...
    if (entry.type === "summary") {
      ...
      // ContentReplacementEntry.agentId is loosely-typed in the log union;
      // narrow at the boundary where we hand it to the path builder.
      const replacementAgentId = entry.agentId as AgentId | undefined;
      const targetFile = replacementAgentId
        ? getAgentTranscriptPath(replacementAgentId)
        : sessionFile;
      ...
      if (entry.type === "queue-operation") {
        ...
        // All other entry types have been handled above. The wide log union
        // doesn't carry isSidechain/agentId/uuid on every variant, so cast
        // here to a shape that exposes the common transcript-message fields.
        type TranscriptEntry = TranscriptMessage & {
          isSidechain?: boolean;
          agentId?: unknown;
          uuid: UUID;
        };
        const txEntry = entry as TranscriptEntry;
        const isAgentSidechain = txEntry.isSidechain && txEntry.agentId !== undefined;
        const targetFile = isAgentSidechain
          ? getAgentTranscriptPath(asAgentId(txEntry.agentId as AgentId))
          : sessionFile;
        ...
        const isNewUuid = !messageSet.has(txEntry.uuid);
...
export function checkResumeConsistency(chain: Message[]): void {
  for (let i = chain.length - 1; i >= 0; i--) {
    ...
    // turn_duration subtype carries messageCount via SystemTurnDurationMessage.
    // SystemMessage's discriminated union widens to its base shape on subtype
    // narrowing; cast to the specific subtype to access the extra field.
    const turn = m as SystemTurnDurationMessage;
    const expected = turn.messageCount;
