...
export function isPathInSandboxWriteAllowlist(resolvedPath: string): boolean {
  ...
  // lodash memoize collapses the return type to unknown when imported via
  // the per-method ESM path; assert at the flatMap boundary so the
  // pathInWorkingPath(string, string) calls below type-check.
  const resolveSandbox = getResolvedSandboxConfigPath as (p: string) => string[];
  const resolvedAllow = allowOnly.flatMap(resolveSandbox);
  const resolvedDeny = denyWithinAllow.flatMap(resolveSandbox);
...
export function isPathAllowed(
  ...
  if (operationType !== "read") {
    ...
    if (!safetyCheck.safe) {
      // Discriminator narrowing on the {safe: true|false} union doesn't
      // reach .message / .classifierApprovable through the !.safe flow
      // under strict:false; cast to the failure variant locally.
      const unsafe = safetyCheck as {
        safe: false;
        message: string;
