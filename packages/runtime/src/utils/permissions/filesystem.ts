...
): { safe: boolean; message?: string; classifierApprovable?: boolean } {
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  if (!safetyCheck.safe) {
    ...
    // SafetyCheckResult discriminator narrowing on !.safe doesn't reach
    // the failure-only fields under strict:false; cast locally.
