...
export default function sliceAnsi(str: string, start: number, end?: number): string {
  ...
  for (const token of tokens) {
    ...
    // ansi-tokenize's Char union (control-code | character) doesn't surface
    // {fullWidth, value} on ControlCode; widen at the access boundary —
    // the .type === "ansi" check above branches; this branch is the Char
    // variant which carries both fields.
    ...
    if (token.type === "ansi") {
      ...
      if (include) {
        // Same Char-variant cast — token is in the else branch so .value exists.
