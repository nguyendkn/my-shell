...
// strictNullChecks) — discriminated unions on `ok` don't narrow the
// optional reason field off the unselected branch. Callers must check
// `ok` before reading `reason`.
export type SemanticCheckResult = { ok: boolean; reason?: string };
