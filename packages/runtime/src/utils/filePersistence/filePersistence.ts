...
async function executeBYOCPersistence(
  ...
  for (const result of results) {
    if (result.success) {
      ...
      // TS narrowing on the discriminated UploadResult union doesn't reach
      // the error field under this package's strict:false config; cast to
      // the failure variant explicitly.
      const failed = result as { path: string; error: string; success: false };
      failedFiles.push({
