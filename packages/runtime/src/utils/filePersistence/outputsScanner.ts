...
export async function findModifiedFiles(
  ...
  // Use recursive flag to get all entries in one call. readdir's overload
  // resolution differs between @types/node and @types/bun (Dirent<string>
  // vs Dirent<NonSharedBuffer>); widen to import('node:fs').Dirent[] so the
  // .name string operations type-check across both.
