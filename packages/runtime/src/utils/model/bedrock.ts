...
const AWS_BEDROCK_CLIENT = "@aws-sdk/client-bedrock";
const AWS_NODE_HTTP_HANDLER = "@smithy/node-http-handler";
const SMITHY_CORE = "@smithy/core";
...
async function createBedrockClient() {
  const { BedrockClient } = await import(AWS_BEDROCK_CLIENT);
  ...
  const clientConfig: ConstructorParameters<typeof BedrockClient>[0] = {
    ...
    ...(skipAuth && {
      requestHandler: new (await import(AWS_NODE_HTTP_HANDLER)).NodeHttpHandler(),
      httpAuthSchemes: [
        {
          schemeId: "smithy.api#noAuth",
          identityProvider: () => async () => ({}),
          signer: new (await import(SMITHY_CORE)).NoAuthSigner(),
        },
      ],
  ...
  if (!skipAuth && !process.env.AWS_BEARER_TOKEN_BEDROCK) {
    ...
    if (cachedCredentials) {
      (clientConfig as { credentials?: unknown }).credentials = {
