...
export function* normalizeMessage(message: Message): Generator<SDKMessage> {
  switch (message.type) {
    case "assistant":
      ...
      return;
    case "progress": {
      // ProgressMessage<T = unknown> — message.data is unknown by default.
      // Runtime invariant: progress messages emitted by the runtime carry
      // a discriminated payload from src/types/tools.ts (ToolProgressData)
      // or HookProgress. Narrow at the boundary so the .type / .message /
      // .elapsedTimeSeconds / .taskId accesses below type-check.
      const data = message.data as
        | { type: string; message?: Message; elapsedTimeSeconds?: number; taskId?: string }
        | undefined;
      if (!data) break;
      ...
      } else if (data.type === "bash_progress" || data.type === "powershell_progress") {
