...
const AWS_CREDENTIAL_PROVIDER_NODE = "@aws-sdk/credential-provider-node";
const SMITHY_NODE_HTTP_HANDLER = "@smithy/node-http-handler";
...
export async function getAWSClientProxyConfig(): Promise<object> {
  ...
  const [{ NodeHttpHandler }, { defaultProvider }] = await Promise.all([
    import(SMITHY_NODE_HTTP_HANDLER),
    import(AWS_CREDENTIAL_PROVIDER_NODE),
  ]);
