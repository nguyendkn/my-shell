...
function extractSideQuestionResponse(messages: Message[]): string | null {
  ...
  if (apiErr) {
    // SystemAPIErrorMessage["error"] widens to `string | APIError`; the
    // formatter only accepts the structured variant, so fall back to the
    // string when present.
    const err = apiErr.error;
    return `(API error: ${typeof err === "string" ? err : formatAPIError(err)})`;
  }
