...
/**
 * Tag for the queue lifecycle operation that produced a log entry. Local to
 * this module so we don't widen the public messageQueueTypes surface; mirrors
 * the literal strings passed below to `recordQueueOperation`.
 */
type QueueOperation = "enqueue" | "dequeue" | "remove" | "popAll";
...
// The tuple contains every non-editable mode; Exclude<> on the other side
// keeps this exhaustive — adding a new `*-notification` mode forces us to
// extend the tuple (TS errors until the literal is listed).
const NON_EDITABLE_MODES_LIST: readonly Exclude<PromptInputMode, EditablePromptInputMode>[] = [
  "task-notification",
];
const NON_EDITABLE_MODES = new Set<PromptInputMode>(NON_EDITABLE_MODES_LIST);
