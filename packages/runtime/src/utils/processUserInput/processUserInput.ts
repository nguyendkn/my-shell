...
export async function processUserInput({
  ...
  for await (const hookResult of executeUserPromptSubmitHooks(
    ...
    if (hookResult.message) {
      // HookResult.message is the AttachmentMessage|HookResultMessage|
      // ProgressMessage union. Only AttachmentMessage carries the
      // .attachment field; route the others through the default branch.
      const hookMsg = hookResult.message;
      if (hookMsg.type === "attachment") {
        switch (hookMsg.attachment.type) {
          case "hook_success":
            if (!("content" in hookMsg.attachment) || !hookMsg.attachment.content) {
            ...
            result.messages.push({
              ...
              attachment: {
                ...hookMsg.attachment,
                content: applyTruncation(hookMsg.attachment.content),
              },
            });
            break;
          default:
            result.messages.push(hookMsg);
            break;
        }
      }
      // Progress messages are filtered out at the loop top (line 173); the
      // remaining narrowed type after the attachment branch is only
      // HookResultMessage, so no further dispatch is needed.
    }
  }
