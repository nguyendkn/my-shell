...
export class ExitPlanModeScanner {
  ...
  ingest(newEvents: SDKMessage[]): ScanResult {
    for (const m of newEvents) {
      if (m.type === "assistant") {
        // SDK assistant message inner shape is `unknown`; the content array
        // structurally matches the typed-block surface this scanner needs.
        const assistantContent = (m.message as { content?: readonly unknown[] }).content ?? [];
        for (const block of assistantContent as readonly { type?: string }[]) {
