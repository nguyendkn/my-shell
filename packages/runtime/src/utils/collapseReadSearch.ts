...
function getCollapsibleToolInfo(
  ...
  if (msg.type === "grouped_tool_use") {
    ...
    if (firstContent?.type === "tool_use" && msg.toolName) {
  ...
  if (msg.type === "user") {
    ...
    const toolResults = content.filter(
      (c): c is { type: "tool_result"; tool_use_id: string } =>
        typeof c !== "string" && c.type === "tool_result",
    );
...
export function collapseReadSearchGroups(
  ...
  for (const msg of messages) {
    if (isCollapsibleToolUse(msg, tools)) {
      ...
      const hookInfos = msg.hookInfos ?? [];
      currentGroup.hookCount += msg.hookCount ?? 0;
      currentGroup.hookTotalMs +=
        msg.totalDurationMs ?? hookInfos.reduce((sum, h) => sum + (h.durationMs ?? 0), 0);
