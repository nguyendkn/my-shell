...
export async function loadPluginLspServers(
  ...
  // 2. Check manifest.lspServers field. The manifest schema's inferred
  // type is structurally the same as LspServerConfig but the two schemas
  // resolve to distinct nominal types — cast at the boundary so the loader
  // signature stays authoritative.
  if (plugin.manifest.lspServers) {
    const manifestServers = await loadLspServersFromManifest(
      plugin.manifest.lspServers as Parameters<typeof loadLspServersFromManifest>[0],
      plugin.path,
      plugin.name,
      errors,
    );
...
async function loadLspServersFromManifest(
  ...
  for (const decl of declarations) {
    if (typeof decl === "string") {
      ...
      // Inline configs. The schema's parsed shape matches LspServerConfig
      // structurally but resolves to a different nominal type; cast at the
      // assignment boundary so the servers map keeps its public type.
      for (const [serverName, config] of Object.entries(decl)) {
        const result = LspServerConfigSchema().safeParse(config);
        if (result.success) {
          servers[serverName] = result.data as (typeof servers)[string];
        } else {
