...
export async function installResolvedPlugin({
  ...
  if (!resolution.ok) {
    // Resolution narrowed to the failure variants of ResolutionResult by
    // !resolution.ok; cast at the return boundary so the discriminator
    // intersection on InstallCoreResult lines up.
    return {
      ok: false,
      reason: "resolution-failed",
 ...
 */
// Flattened to a single shape because tsconfig has strict:false; callers
// check `success` before reading the optional message/error fields.
...
export async function installPluginFromMarketplace({
  ...
  try {
    ...
    if (!result.ok) {
      // TS doesn't narrow InstallCoreResult through the !result.ok flow
      // analysis under the looser strict config used by this package; the
      // failure variants all share the discriminator { ok: false; reason: ... }
      // so cast once here to a typed failure result and switch normally.
      const failure = result as Extract<InstallCoreResult, { ok: false }>;
    ...
    // After the !result.ok early-return above, result is the success
    // variant of InstallCoreResult.
