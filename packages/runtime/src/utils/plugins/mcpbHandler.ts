// @anthropic-ai/mcpb 2.x renamed `McpbManifest` to `McpbManifestAny` and
// stopped exporting `McpbUserConfigurationOption`. Alias the new name and
// derive the user-config option shape from the inferred manifest type so
// downstream callers keep their original type names.
import type { McpbManifestAny } from "@anthropic-ai/mcpb";

export type McpbManifest = McpbManifestAny;
export type McpbUserConfigurationOption = NonNullable<McpbManifestAny["user_config"]>[string];
