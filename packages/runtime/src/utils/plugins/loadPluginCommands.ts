...
export const getPluginCommands = memoize(async (): Promise<Command[]> => {
  ...
  const perPluginCommands = await Promise.all(
    enabled.map(async (plugin): Promise<Command[]> => {
            ...
            try {
              ...
              if (stats.isDirectory()) {
                ...
                if (plugin.commandsMetadata) {
                  ...
                  for (const [name, untypedMeta] of Object.entries(plugin.commandsMetadata)) {
                    // Object.entries inference can collapse a refined Zod
                    // schema's value type to unknown; reassert the inferred
                    // CommandMetadata shape locally so .source access type-checks.
                    const metadata = untypedMeta as CommandMetadata;
                    if (metadata.source) {
      ...
      if (plugin.commandsMetadata) {
        for (const [name, untypedMeta] of Object.entries(plugin.commandsMetadata)) {
          // See note above the previous loop: reassert CommandMetadata shape
          // because Object.entries collapses Zod-refine inferred value types.
          const metadata = untypedMeta as CommandMetadata;
