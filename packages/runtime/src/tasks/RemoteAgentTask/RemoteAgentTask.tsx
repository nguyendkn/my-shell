...
// SDKAssistantMessage["message"] is typed as `unknown` because the schema
// uses a placeholder for @anthropic-ai/sdk's APIAssistantMessage. We only
// need the structural `.content` array of typed blocks here.
type AssistantContentBlock = { type: string; name?: string; input?: unknown; text?: string };
type AssistantMessageView = { content: readonly AssistantContentBlock[] };

function assistantContent(msg: SDKAssistantMessage): readonly AssistantContentBlock[] {
  return (msg.message as AssistantMessageView).content;
}
...
// Precondition error result. Flattened to a single shape because the
// runtime tsconfig has strict:false (no strictNullChecks) — discriminated
// unions on `eligible` don't narrow the optional fields off the unselected
// branch. Callers must check `eligible` before reading `errors`.
...
export function extractPlanFromLog(log: SDKMessage[]): string | null {
  ...
  for (let i = log.length - 1; i >= 0; i--) {
    ...
    const fullText = extractTextContent(assistantContent(msg), "\n");
...
function extractReviewFromLog(log: SDKMessage[]): string | null {
  ...
  const allText = log
    ...
    .map((msg) => extractTextContent(assistantContent(msg), "\n"))
    .join("\n")
    .trim();
  return allText || null;
}
...
function extractTodoListFromLog(log: SDKMessage[]): TodoList {
  ...
  const input = assistantContent(todoListMessage).find(
