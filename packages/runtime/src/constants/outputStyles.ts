...
export async function getOutputStyleConfig(): Promise<OutputStyleConfig | null> {
  ...
  // Check for forced plugin output styles. memoize() loses the
  // value-type, so the Object.values() callback receives `unknown`; cast
  // to the known shape this filter walks before reading source/forceForPlugin.
  const forcedStyles = Object.values(allStyles).filter((style): style is OutputStyleConfig => {
    const s = style as (OutputStyleConfig & { source?: string; forceForPlugin?: boolean }) | null;
    return s !== null && s.source === "plugin" && s.forceForPlugin === true;
  });
