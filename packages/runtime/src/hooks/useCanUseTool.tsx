                ...
                if (
                  ...
                  if (speculativePromise) {
                    const raceResult = await Promise.race<
                      RaceResult<ClassifierResult> | RaceTimeout
                    >([
                      speculativePromise.then(_temp<ClassifierResult>),
                      new Promise<RaceTimeout>(_temp2),
                    ]);
                    if (ctx.resolveIfAborted(resolve)) {
                      return;
                    }
...
type RaceTimeout = { type: "timeout" };
type RaceResult<R> = { type: "result"; result: R };

function _temp2(res: (value: RaceTimeout) => void): ReturnType<typeof setTimeout> {
  return setTimeout(res, 2000, { type: "timeout" as const });
}
function _temp<R>(r: R): RaceResult<R> {
  return {
