    ...
    const manager = new RemoteSessionManager(config, {
      onMessage: (sdkMessage) => {
        ...
        if (sdkMessage.type === "user") {
          // SDK message inner shape is `unknown`; narrow to the structural
          // surface this debug log walks (content array of typed blocks).
          const c = (sdkMessage.message as { content?: unknown } | undefined)?.content;
          parts.push(
            `content=${
              Array.isArray(c) ? c.map((b: { type?: string }) => b.type ?? "?").join(",") : typeof c
            }`,
          );
        }
