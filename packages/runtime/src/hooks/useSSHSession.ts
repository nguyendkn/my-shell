  ...
  useEffect(() => {
    ...
    const manager = session.createManager({
      onMessage: (rawMessage) => {
        // SSHManagerCallbacks.onMessage typed as (unknown) so the SSH
        // session module doesn't depend on the SDK types. Cast at the
        // hook boundary — the SSH server forwards proper SDKMessage
        // payloads as an invariant of the streaming protocol.
      ...
      onPermissionRequest: (request, requestId) => {
        ...
        // SSHPermissionRequest is structurally identical to the SDK control
        // request the synthetic-message helper expects, but they're distinct
        // nominal types. Pass through Parameters<...> so the helper signature
        // stays authoritative.
        const syntheticMessage = createSyntheticAssistantMessage(
          request as Parameters<typeof createSyntheticAssistantMessage>[0],
          requestId,
        );
