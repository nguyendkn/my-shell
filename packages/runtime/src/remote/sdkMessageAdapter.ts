 ...
 */
// Wire UUIDs are plain strings; the runtime contract is the branded UUID
// template literal type. Narrow at the boundary in one place so the rest of
// this file doesn't repeat the cast.
...
function convertAssistantMessage(msg: SDKAssistantMessage): AssistantMessage {
  return {
    type: "assistant",
    // SDKAssistantMessage.message is wire-typed (APIAssistantMessagePlaceholder
    // = z.unknown()); the runtime invariant is the SDK assistant message
    // shape that AssistantMessageInner mirrors.
    message: msg.message as AssistantMessageInner,
    uuid: asUUID(msg.uuid),
    requestId: undefined,
...
function convertStreamEvent(msg: SDKPartialAssistantMessage): StreamEvent {
  return {
    type: "stream_event",
    // The wire schema declares event with z.unknown(); narrow at the
    // adapter boundary to the SDK's BetaRawMessageStreamEvent shape.
...
export function convertSDKMessage(msg: SDKMessage, opts?: ConvertOptions): ConvertedMessage {
  switch (msg.type) {
    ...
    case "user": {
      // SDKUserMessage.message is wire-typed unknown; runtime invariant is
