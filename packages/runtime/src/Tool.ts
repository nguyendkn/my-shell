...
// Flattened to a single shape because tsconfig has strict:false (no
// strictNullChecks) — discriminated unions on `result` don't narrow the
// optional fields off the unselected branch. Callers must check `result`
// before reading `message` / `errorCode`.
...
export function filterToolProgressMessages(
  ...
  return progressMessagesForMessage.filter(
    (msg): msg is ProgressMessage<ToolProgressData> =>
      (msg.data as { type?: string } | undefined)?.type !== "hook_progress",
  );
}
