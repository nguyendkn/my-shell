...
function PromptInput({
  ...
  const hasTungstenSession = useAppState(
    (s) => ("external" as string) === "ant" && s.tungstenActiveSession !== undefined,
  );
  ...
  const hasBgTaskPill = useMemo(
    () =>
      Object.values(tasks).some(
        (t: TaskState) =>
          isBackgroundTask(t) && !(("external" as string) === "ant" && isPanelAgentTask(t)),
      ),
    [tasks],
  );
  ...
  const cachedTeams: TeamSummary[] = useMemo(() => {
    ...
    const teammateCount = count(
      Object.values(teamContext.teammates) as ReadonlyArray<{ name: string }>,
      (t) => t.name !== "team-lead",
    );
    return [
      {
  ...
  const runningTaskCount = useMemo(
    () => count(Object.values(tasks) as TaskState[], (t) => t.status === "running"),
    [tasks],
  );
  ...
  const tasksFooterVisible =
    (runningTaskCount > 0 || (("external" as string) === "ant" && coordinatorTaskCount > 0)) &&
  ...
  const memberMentionHighlights = useMemo((): Array<{
    ...
    const regex = /(^|\s)@([\w-]+)/g;
    const memberValues = Object.values(members) as ReadonlyArray<{
      name: string;
      color?: string;
    }>;
    let match;
  ...
  useEffect(() => {
    ...
    setPastedContents((prev) => {
      const values = Object.values(prev) as PastedContent[];
      const orphaned = values.filter((c) => c.type === "image" && !referencedIds.has(c.id));
