...
export function filterForBriefTool<T extends BriefFilterMessageLike>(
  messages: T[],
  ...
  return messages.filter((msg) => {
    ...
    const innerMessage = msg.message as { content?: unknown } | undefined;
    const rawContent = innerMessage?.content;
    const block = (Array.isArray(rawContent) ? rawContent[0] : undefined) as BriefBlock | undefined;
 ...
 */
type DropTextMessageLike = {
  type: string;
  isMeta?: boolean;
  message?: unknown;
};

export function dropTextInBriefTurns<T extends DropTextMessageLike>(
  messages: T[],
  briefToolNames: string[],
): T[] {
  const nameSet = new Set(briefToolNames);
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    const innerMessage = msg.message as { content?: unknown } | undefined;
    const rawContent = innerMessage?.content;
    const block = (Array.isArray(rawContent) ? rawContent[0] : undefined) as
...
const MessagesImpl = ({
  ...
  const isItemClickable = useCallback(
    (msg_6: RenderableMessage): boolean => {
      ...
      const userContent = msg_6.message.content;
      if (typeof userContent === "string") return false;
      const b_0 = userContent[0];
      if (
        !b_0 ||
        typeof b_0 === "string" ||
        b_0.type !== "tool_result" ||
        b_0.is_error ||
        !msg_6.toolUseResult
      ) {
        return false;
