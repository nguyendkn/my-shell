...
function computeStickyPromptText(msg: RenderableMessage): string | null {
  ...
  if (msg.type === "user") {
    ...
    const block = Array.isArray(userContent) ? userContent[0] : undefined;
...
export function VirtualMessageList({
  ...
  useImperativeHandle(cursorNavRef, (): MessageActionsNav => {
    const select = (m: NavigableMessage) =>
      setCursor?.({
        uuid: m.uuid,
        // NavigableMessage covers more discriminants than NavigableType; the
        // navigation cursor only flows visible messages, so cast at the seam.
