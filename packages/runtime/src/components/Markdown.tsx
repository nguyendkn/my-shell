  ...
  if ($[0] !== children || $[1] !== dimColor || $[2] !== highlight || $[3] !== theme) {
    ...
    for (const token of tokens) {
      if (token.type === "table") {
        ...
        // Two different `marked` module copies resolve to structurally
        // identical but nominally-distinct Tokens.Table / Props types; cast
        // the whole JSX expression so TS stops trying to align them.
        const TableCell = MarkdownTable as unknown as (props: unknown) => React.ReactNode;
        elements.push(<TableCell key={elements.length} token={token} highlight={highlight} />);
      } else {
