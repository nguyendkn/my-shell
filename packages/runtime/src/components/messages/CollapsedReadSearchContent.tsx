...
// Local narrowing types for fields the upstream message type exposes as
// `unknown`/`unknown[]`. Each type guard narrows a single shape used by this
// renderer; nothing here mutates shared data.
type CommitEntry = { kind: "committed" | "amended" | "cherry-picked"; sha: string };
type PushEntry = { branch: string };
type BranchEntry = { action: "merged" | "rebased"; ref: string };
type PrEntry = {
  action: "created" | "edited" | "merged" | "commented" | "closed" | "ready";
  number: number;
  url?: string;
};
type MemoryEntry = { path: string; content: string };
...
function isReplStartProgress(value: unknown): value is ReplToolCallProgress {
  return (
    hasType(value) &&
    (value as { type: string }).type === "repl_tool_call" &&
    "phase" in value &&
    "toolInput" in value &&
    "toolName" in value
  );
...
function isShellProgress(value: unknown): value is ShellProgress {
  ...
  return (
    (t === "bash_progress" || t === "powershell_progress") &&
    "elapsedTimeSeconds" in value &&
    "totalLines" in value
  );
}
...
export function CollapsedReadSearchContent({
  ...
  const searchArgs = asStringArray(message.searchArgs);
  let incomingHint: string | undefined =
    typeof message.latestDisplayHint === "string" ? message.latestDisplayHint : undefined;
  ...
  if (isActiveGroup) {
    for (const id_0 of toolUseIds) {
      ...
      if (isReplStartProgress(latest) && latest.phase === "start") {
  ...
  if (verbose) {
    ...
    return (
      <Box flexDirection="column">
        ...
        {message.relevantMemories?.filter(isMemoryEntry).map((m) => (
  ...
  if (isFullscreenEnvEnabled() && message.commits?.length) {
    ...
    const commitEntries = message.commits.filter(isCommitEntry);
    for (const kind of ["committed", "amended", "cherry-picked"] as const) {
      const shas = commitEntries.filter((c) => c.kind === kind).map((c_0) => c_0.sha);
      if (shas.length) {
  ...
  if (isFullscreenEnvEnabled() && message.pushes?.length) {
    const branches = uniq(message.pushes.filter(isPushEntry).map((p) => p.branch));
  ...
  if (isFullscreenEnvEnabled() && message.branches?.length) {
    ...
    for (const b of message.branches.filter(isBranchEntry)) {
  ...
  if (isFullscreenEnvEnabled() && message.prs?.length) {
    ...
    for (const pr of message.prs.filter(isPrEntry)) {
      pushPart(
