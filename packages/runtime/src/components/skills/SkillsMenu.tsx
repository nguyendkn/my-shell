...
export function SkillsMenu(t0) {
  ...
  const skills = t1;
  // Pin the bucket map so the React-compiler's $[3] cache slot keeps the
  // typed shape — without this the slot reads back as `unknown` and
  // group.sort() fails.
  let groups: Record<SkillSource, typeof skills>;
  if ($[2] !== skills) {
    groups = {
      policySettings: [],
      userSettings: [],
      ...
      plugin: [],
      mcp: [],
    } as Record<SkillSource, typeof skills>;
