...
export function isNavigableMessage(msg: NavigableMessage): boolean {
  switch (msg.type) {
    ...
    case "user": {
      ...
      const b = Array.isArray(userContent) ? userContent[0] : undefined;
      if (!b || typeof b === "string" || b.type !== "text") return false;
...
export function copyTextOf(msg: NavigableMessage): string {
  switch (msg.type) {
    case "user": {
      const userContent = msg.message.content;
      const b = Array.isArray(userContent) ? userContent[0] : undefined;
      return b && typeof b !== "string" && b.type === "text" ? stripSystemReminders(b.text) : "";
    }
    case "assistant": {
      const b = msg.message.content[0];
...
function toolResultText(r: NormalizedUserMessage): string {
  const userContent = r.message.content;
  const b = Array.isArray(userContent) ? userContent[0] : undefined;
  if (!b || typeof b === "string" || b.type !== "tool_result") return "";
  const c = b.content;
