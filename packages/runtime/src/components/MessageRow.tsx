...
export function hasContentAfterIndex(
  ...
  for (let i = index + 1; i < messages.length; i++) {
    ...
    if (msg?.type === "user") {
      const content = msg.message.content[0];
      // user.message.content is `string | ContentBlockParam[]` — only block
      // arrays carry a tool_result; raw strings cannot.
      if (typeof content === "object" && content && content.type === "tool_result") {
        continue;
      }
    }
    ...
    if (msg?.type === "grouped_tool_use") {
      const firstBlock = msg.messages[0]?.message.content[0] as { input?: unknown } | undefined;
      if (getToolSearchOrReadInfo(msg.toolName, firstBlock?.input, tools).isCollapsible) {
        continue;
      }
    }
    return true;
...
export function isMessageStreaming(
  ...
  // After the grouped/collapsed branches, msg is a NormalizedMessage.
