...
export function MCPRemoteServerMenu({
  ...
  const handleAuthenticate = React.useCallback(async () => {
    ...
    try {
      // Narrow stdio AND claudeai-proxy out — performMCPOAuthFlow /
      // revokeServerTokens accept only sse | http. The early
      // `claudeai-proxy` return above narrows runtime, but TS doesn't track
      // it across the closure boundary, so re-narrow explicitly here.
      const remoteConfig =
        server.config && (server.config.type === "sse" || server.config.type === "http")
          ? (server.config as Extract<typeof server.config, { type: "sse" | "http" }>)
          : null;
      ...
      if (server.isAuthenticated && remoteConfig) {
        await revokeServerTokens(server.name, remoteConfig, {
  ...
  const handleClearAuth = async () => {
    if (server.config.type === "claudeai-proxy") return;
    if (server.config && (server.config.type === "sse" || server.config.type === "http")) {
      const remoteConfig_1 = server.config as Extract<
        typeof server.config,
        { type: "sse" | "http" }
      >;
      // First revoke the authentication tokens and clear all auth state
      await revokeServerTokens(server.name, remoteConfig_1);
  ...
  if (isAuthenticating) {
    ...
    const oauthCfg =
      cfg.type === "sse" || cfg.type === "http"
        ? (cfg as Extract<typeof cfg, { type: "sse" | "http" }>).oauth
        : undefined;
    const authCopy = oauthCfg?.xaa
    ...
    <Box flexDirection="column">
      <Box flexDirection="column" paddingX={1} borderStyle={borderless ? undefined : "round"}>
        ...
        <Box flexDirection="column" gap={0}>
          ...
          <Box>
            <Text bold>URL: </Text>
            <Text dimColor>{"url" in server.config ? server.config.url : ""}</Text>
          </Box>
