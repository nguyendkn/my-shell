...
export async function installPluginOp(
  ...
  if (!result.ok) {
    // See pluginInstallationHelpers note: TS doesn't narrow InstallCoreResult
    // across the !result.ok flow under this package's strict:false config.
    const failure = result as Extract<InstallCoreResult, { ok: false }>;
    switch (failure.reason) {
      case "local-source-no-location":
        return {
          success: false,
          message: `Cannot install local plugin "${failure.pluginName}" without marketplace install location`,
        };
      case "settings-write-failed":
        return {
          success: false,
          message: `Failed to update settings: ${failure.message}`,
        };
      case "resolution-failed":
        return {
          success: false,
          message: formatResolutionError(failure.resolution),
        };
      case "blocked-by-policy":
        return {
          success: false,
          message: `Plugin "${failure.pluginName}" is blocked by your organization's policy and cannot be installed`,
        };
      case "dependency-blocked-by-policy":
        return {
          success: false,
          message: `Plugin "${failure.pluginName}" depends on "${failure.blockedDependency}", which is blocked by your organization's policy`,
        };
    }
  }
