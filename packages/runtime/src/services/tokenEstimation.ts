...
// Loose shape covering AssistantMessage / UserMessage / AttachmentMessage /
// SystemMessage. SystemMessage carries `message?: string` (vs the
// content-bearing object on assistant/user) so the field is typed `unknown`
// here and narrowed at the use site by checking message.type first.
...
export function roughTokenCountEstimationForMessage(message: TokenCountableMessage): number {
  const inner =
    typeof message.message === "object" && message.message !== null
      ? (message.message as { content?: unknown })
      : undefined;
  if ((message.type === "assistant" || message.type === "user") && inner?.content) {
...
function roughTokenCountEstimationForBlock(
  ...
  if (block.type === "tool_result") {
    // tool_result.content is a narrower union (TextBlockParam | ImageBlockParam |
    // DocumentBlockParam | SearchResultBlockParam | ToolReferenceBlockParam)[]
    // — all variants are subsets of ContentBlockParam, so widening here is
    // sound. The cast bridges the SDK's stricter input shape to the union the
    // estimator handles.
    return roughTokenCountEstimationForContent(
      block.content as string | Anthropic.ContentBlockParam[],
    );
  }
  if (block.type === "tool_use") {
