...
export async function getSessionLogs(sessionId: string, url: string): Promise<Entry[] | null> {
  ...
  if (logs && logs.length > 0) {
    // Update our lastUuid to the last entry's UUID. Entry.uuid widens to
    // `unknown` after the discriminator narrowing falls through; cast to
    // the branded UUID literal the map expects.
    const lastEntry = logs.at(-1);
    if (lastEntry && "uuid" in lastEntry && lastEntry.uuid) {
      lastUuidMap.set(sessionId, lastEntry.uuid as Parameters<typeof lastUuidMap.set>[1]);
    }
  }
