...
export async function revokeServerTokens(
  ...
  if (preserveStepUpState && tokenData && (tokenData.stepUpScope || tokenData.discoveryState)) {
    ...
    const updatedData: SecureStorageData = {
      ...freshData,
      mcpOAuth: {
        ...(freshData.mcpOAuth as Record<string, Record<string, unknown>> | undefined),
        [serverKey]: {
...
async function performMCPXaaAuth(
  ...
  try {
    ...
    storage.update({
      ...existingData,
      mcpOAuth: {
        ...(existingData.mcpOAuth as Record<string, Record<string, unknown>> | undefined),
        [serverKey]: {
          ...prev,
          serverName,
          serverUrl: serverConfig.url,
...
export function saveMcpClientSecret(
  ...
  storage.update({
    ...
    mcpOAuthClientConfig: {
      ...(existingData.mcpOAuthClientConfig as Record<string, unknown> | undefined),
