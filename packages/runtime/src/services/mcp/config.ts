...
export function parseMcpConfig(params: {
  ...
  for (const [name, config] of Object.entries(schemaResult.data.mcpServers)) {
    ...
    // Check for Windows-specific npx usage without cmd wrapper. Narrow to
    // the stdio variant (only that variant has command); the if-branch
    // gate above guarantees this at runtime.
    const stdioCfg = configToCheck as { type?: string; command: string };
    if (
      getPlatform() === "windows" &&
      (!stdioCfg.type || stdioCfg.type === "stdio") &&
