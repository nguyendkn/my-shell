  ...
  async (
    ...
    try {
      ...
      if (serverRef.type === "sse") {
        ...
        ((serverRef as { type?: string }).type === "stdio" ||
          !(serverRef as { type?: string }).type) &&
        ...
        // After narrowing through every other variant in the union, TS thinks
        // serverRef is `never`. The runtime invariant is the stdio variant
        // here (type missing or "stdio"); cast to access command/args/env.
        const stdio = serverRef as {
          command: string;
          args: string[];
          env?: Record<string, string>;
        };
        const finalCommand = process.env.CLAUDE_CODE_SHELL_PREFIX || stdio.command;
        const finalArgs = process.env.CLAUDE_CODE_SHELL_PREFIX
          ? [[stdio.command, ...stdio.args].join(" ")]
          : stdio.args;
