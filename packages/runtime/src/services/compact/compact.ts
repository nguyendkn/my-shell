...
export interface CompactionResult {
  ...
  hookResults: HookEmittedMessage[];
...
async function streamCompactSummary({
  ...
  try {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      // Use for-await on the generator directly so TS narrows the yielded
      // value. The manual streamIter.next() form leaks the AsyncGenerator's
      // TReturn=void into next.value, forcing every property access to deal
      // with the void branch even after the !next.done check.
