      ...
      switch (message.type) {
        ...
        case "system": {
          ...
          if (message.subtype === "api_error") {
            // Narrow to SystemAPIErrorMessage so the retry-meta fields
            // (retryAttempt/maxRetries/retryInMs/error) are accessible —
            // SystemMessage's subtype field is a free string and doesn't
            // discriminate the union by itself.
            const apiErr = message as SystemAPIErrorMessage;
            const apiError = apiErr.error;
            // apiError may be the SDK APIError instance or a string code
            // (max_output_tokens etc); only the structured form has .status.
            const errorStatus =
              apiError && typeof apiError === "object" && "status" in apiError
                ? ((apiError as { status?: number }).status ?? null)
                : null;
            yield {
              type: "system",
              subtype: "api_retry" as const,
              attempt: apiErr.retryAttempt,
              max_retries: apiErr.maxRetries,
              retry_delay_ms: apiErr.retryInMs,
              error_status: errorStatus,
              // categorizeRetryableAPIError expects the structured APIError;
              // SystemAPIErrorMessage's error union also allows the synthetic
              // string code variant, but at the api_error subtype it's
              // always the structured form (createSystemAPIErrorMessage
              // takes APIError directly).
              error: categorizeRetryableAPIError(apiError as APIError),
              session_id: getSessionId(),
              uuid: message.uuid,
            };
          }
