...
//
// Subtypes consumed by the runtime's control-request dispatch in print.ts but
// not part of the public SDK control protocol. They're emitted by the
// runtime's internal harnesses (CCR, OAuth flows, channel management) over
// the same stdin transport as the SDK control union.
// ============================================================================
...
export const SDKControlEndSessionRequestSchema = lazySchema(() =>
  z
    ...
    .describe("Aborts the in-flight conversation and signals session shutdown."),
);
...
export const SDKControlChannelEnableRequestSchema = lazySchema(() =>
  z
    ...
    .describe("Enable a SamplingChannel for the named MCP server."),
);
...
export const SDKControlMcpAuthenticateRequestSchema = lazySchema(() =>
  z
    ...
    .describe("Start an OAuth flow for the named MCP server."),
);
...
export const SDKControlMcpOAuthCallbackUrlRequestSchema = lazySchema(() =>
  z
    ...
    .describe("Deliver the OAuth callback URL for an in-flight MCP authenticate flow."),
);
...
export const SDKControlMcpClearAuthRequestSchema = lazySchema(() =>
  z
    ...
    .describe("Clear cached MCP OAuth credentials for the named server."),
);
...
export const SDKControlClaudeAuthenticateRequestSchema = lazySchema(() =>
  z
    .object({
      ...
      // True when initiating the claude.ai (Pro/Team) login variant rather
      // than the API-key OAuth flow.
      loginWithClaudeAi: z.boolean().optional(),
    })
    .describe("Start the Anthropic Claude OAuth flow."),
);
...
export const SDKControlClaudeOAuthCallbackRequestSchema = lazySchema(() =>
  z
    ...
    .describe("Deliver the Claude OAuth authorization code + state for the active flow."),
);
...
export const SDKControlClaudeOAuthWaitForCompletionRequestSchema = lazySchema(() =>
  z
    ...
    .describe("Block until the active Claude OAuth flow resolves."),
);
...
export const SDKControlGenerateSessionTitleRequestSchema = lazySchema(() =>
  z
    .object({
      ...
      // Optional caller-provided description that biases title generation.
      description: z.string().optional(),
      // When true, persist the generated title alongside the session.
      persist: z.boolean().optional(),
    })
    .describe("Trigger out-of-band session title generation."),
);
...
export const SDKControlSideQuestionRequestSchema = lazySchema(() =>
  z
    ...
    .describe("Ask a side question off the main conversation."),
);
...
export const SDKControlRemoteControlRequestSchema = lazySchema(() =>
  z
    ...
    .describe("Toggle the remote-control bridge for this session."),
);
...
export const SDKControlRequestInnerSchema = lazySchema(() =>
  z.union([
    ...
    // Internal subtypes — runtime-only control messages dispatched in
    // print.ts but not part of the public SDK control protocol.
